...
CREATE TABLE `sys_file` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `original_name` varchar(255) NOT NULL COMMENT '原始文件名(供老师查看时辨认)',
  `file_key` varchar(255) NOT NULL COMMENT '云平台OSS的Object Key(用于管理与删除)',
  `file_url` varchar(500) NOT NULL COMMENT '文件的公网/内网访问链接',
  `file_md5` varchar(64) DEFAULT NULL COMMENT '文件MD5值(用于实现"秒传")',
  `upload_by` bigint(20) NOT NULL COMMENT '上传者的教师ID(关联 teacher 表)',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
(1, 'admin', 'e10adc3949ba59abbe56e057f20f883e', '超级管理员', '13800000000', '信息中心', 1),
(2, 'T202401', 'e10adc3949ba59abbe56e057f20f883e', '张老师', '13911112222', '计算机学院', 0),
(3, 'T202402', 'e10adc3949ba59abbe56e057f20f883e', '李老师', '13933334444', '理学院', 0);
...
-- 【新增】插入文件管理 Mock 数据 (张老师上传了一张课堂图片)
INSERT INTO `sys_file` (`id`, `original_name`, `file_key`, `file_url`, `file_md5`, `upload_by`) VALUES
(1, 'classroom_sample.jpg', 'image/2024/05/sample.jpg', '/uploads/mock/classroom_sample.jpg', 'e10adc3949ba59abbe56e057f20f883e', 2);

-- 插入一条模拟的 AI 分析任务 (关联了张老师，以及刚才插入的 file_id = 1)
INSERT INTO `analysis_task` (`id`, `teacher_id`, `schedule_id`, `file_id`, `media_type`, `status`, `attendance_count`, `total_score`) VALUES
(1, 2, 1, 1, 1, 2, 48, 82.50);
