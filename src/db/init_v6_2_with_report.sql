...
CREATE TABLE `analysis_task` (
  ...
  `file_id` bigint(20) DEFAULT NULL COMMENT '关联 sys_file 的ID',
...
CREATE TABLE `analysis_report` (
  ...
  `schedule_id` bigint(20) NOT NULL COMMENT '关联排课ID',
  `attendance_rate` decimal(5,2) DEFAULT '0.00' COMMENT '出勤率估计',
  `attendance_score` decimal(5,2) DEFAULT '0.00' COMMENT '出勤评分',
  `focus_score` decimal(5,2) DEFAULT '0.00' COMMENT '专注度评分',
  `interaction_score` decimal(5,2) DEFAULT '0.00' COMMENT '互动度评分',
  `discipline_score` decimal(5,2) DEFAULT '0.00' COMMENT '纪律评分',
  `total_score` decimal(5,2) DEFAULT '0.00' COMMENT '综合评分',
  `report_level` varchar(20) DEFAULT NULL COMMENT '评估等级：优秀/良好/一般/需关注',
  `abnormal_flag` tinyint(4) NOT NULL DEFAULT '0' COMMENT '是否存在明显异常时段',
  `summary_text` varchar(500) DEFAULT NULL COMMENT '课堂评估摘要',
  `suggestion_text` varchar(1000) DEFAULT NULL COMMENT '教学建议',
  `behavior_stats_json` json DEFAULT NULL COMMENT '行为统计结果',
  `trend_data_json` json DEFAULT NULL COMMENT '趋势图数据',
  `abnormal_moments_json` json DEFAULT NULL COMMENT '异常时段与抓拍结果',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_task_id` (`task_id`),
  KEY `idx_schedule_id` (`schedule_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='课堂评估报告快照表';
...
(`id`, `task_id`, `schedule_id`, `attendance_rate`, `attendance_score`, `focus_score`,
 `interaction_score`, `discipline_score`, `total_score`, `report_level`, `abnormal_flag`,
 `summary_text`, `suggestion_text`, `behavior_stats_json`, `trend_data_json`, `abnormal_moments_json`,
 `created_at`, `updated_at`)
VALUES
(
  1, 10001, 1, 93.10, 93.10, 88.00,
  78.00, 90.00, 88.50, '良好', 0,
  '本节课整体课堂秩序较好，学生整体专注度较高，课堂互动行为较少但未出现明显异常波动。',
  '建议在课堂中段增加提问或讨论环节，以进一步提升互动度。',
  JSON_OBJECT(
    '正常听课', JSON_OBJECT('avgCount', 32, 'peakCount', 32, 'ratio', 59.26),
    '书写', JSON_OBJECT('avgCount', 12, 'peakCount', 12, 'ratio', 22.22),
    '阅读', JSON_OBJECT('avgCount', 6, 'peakCount', 6, 'ratio', 11.11),
    '举手', JSON_OBJECT('avgCount', 4, 'peakCount', 4, 'ratio', 7.41)
  ),
  JSON_ARRAY(
    JSON_OBJECT('frameTime', 0, 'focusScore', 90, 'abnormalCount', 0),
    JSON_OBJECT('frameTime', 60, 'focusScore', 88, 'abnormalCount', 0),
    JSON_OBJECT('frameTime', 120, 'focusScore', 86, 'abnormalCount', 0),
    JSON_OBJECT('frameTime', 180, 'focusScore', 84, 'abnormalCount', 0)
  ),
  JSON_ARRAY(),
  '2026-03-10 08:06:35', '2026-03-10 08:06:35'
),
(
  2, 10003, 3, 85.71, 85.71, 80.00,
  72.00, 74.00, 81.00, '良好', 1,
  '本节实时课堂整体表现尚可，但在中段出现玩手机行为，课堂纪律存在一定波动。',
  '建议教师在课堂中段增加巡视和互动提醒，降低异常行为持续时间。',
  JSON_OBJECT(
    '正常听课', JSON_OBJECT('avgCount', 23, 'peakCount', 24, 'ratio', 66.67),
    '玩手机', JSON_OBJECT('avgCount', 3, 'peakCount', 3, 'ratio', 8.33)
  ),
  JSON_ARRAY(
    JSON_OBJECT('frameTime', 30, 'focusScore', 84, 'abnormalCount', 0),
    JSON_OBJECT('frameTime', 60, 'focusScore', 82, 'abnormalCount', 0),
    JSON_OBJECT('frameTime', 90, 'focusScore', 75, 'abnormalCount', 3)
  ),
  JSON_ARRAY(
    JSON_OBJECT('frameTime', 90, 'behaviorType', '玩手机', 'count', 3, 'snapshotUrl', 'https://oss.example.com/snapshot/20260316/task10003_phone_90s.jpg')
  ),
  '2026-03-16 14:05:20', '2026-03-16 14:05:20'
);
